 ...
 */
export async function AusleihFormular({
  id,
  ausgeliehenAn,
  ausgeliehenAm,
}: {
  ...
  if (verliehen) {
    return (
      <div className="rounded-xl border border-rost/40 bg-rost/5 p-4">
        <p className="utility text-[10px] text-rost">{t.ausleihe.verliehen}</p>
        <p className="hand mt-1.5 text-tinte">{t.ausleihe.an(ausgeliehenAn ?? "")}</p>
        <p className="mt-1 text-[13px] text-stein">
          {langesDatum(t, ausgeliehenAm) ?? t.datum.ohneDatum}
          {seitWann(t, ausgeliehenAm) ? ` — ${seitWann(t, ausgeliehenAm)}` : ""}
        </p>
